...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        if config.weight != None:
            self.weights = torch.FloatTensor(config.weight).cuda()
        else:
            self.weights = None
        self.kernel = 16
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.linear = nn.Linear(self.out_channel*(300-self.kernel+1), config.hidden_size)
        self.classifier = nn.Linear(config.hidden_size, self.config.num_labels)
        self.conv = nn.Conv2d(1 ,self.out_channel ,kernel_size=(self.kernel,config.hidden_size))
        self.init_weights()
    ...
    Examples::
        ...
        pooled_output = output.view(batch , -1)
...
class BertForSequenceClassificationVisualize(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        if config.weight != None:
